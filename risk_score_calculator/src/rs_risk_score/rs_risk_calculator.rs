...
pub fn calculate_risk_score(alleles: &Vec<RsAlleles>, scores: HashMap<RsAlleles, f32>) -> f32 {
    ...
    return alleles_per_haplotype.into_iter().map(|allele| get_locus_beta(&allele, &scores)).sum::<f32>();
}
...
fn get_locus_beta(allele: &RsAlleles, scores: &HashMap<RsAlleles, f32>) -> f32 {
